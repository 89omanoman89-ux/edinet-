...
P5で限定的なDuckDB/Parquet取得アダプタ、P5.5で私有Parquet snapshotとread-only照会を実装した。
適用範囲は[私有dataset利用契約](CHATGPT_DATA_USAGE.md)を参照する。全市場DBのDDLや運用基盤は未実装。
...
P3の`financial_views.py`に、限定した累計差分のDAG検証を実装した。親のproxy/未検証を値へ昇格せず、合成フラグを継承する（[適用範囲](P3.md)）。
